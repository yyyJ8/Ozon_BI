...
def _upsert_posting(db: Session, p: dict, store_id: int,
                    delivery_schema: Optional[str] = None) -> bool:
    ...
    stmt = pg_insert(Posting).values(**data).on_conflict_do_update(
        index_elements=["store_id", "posting_number"],
        set_={
            "status": data["status"],
...
def sync_postings(db: Session, client: OzonClient,
    ...
    """
    logger.info(f"=== [store={store_id}] 开始同步订单履约: {date_from} ~ {date_to} ===")
    ...
    for schema in ("FBO", "FBS"):
        try:
            logger.info(f"  [store={store_id}] 增量拉取 {schema} ({date_from} ~ {date_to})...")
            ...
            for p in postings:
                ...
                if _upsert_posting(db, p, store_id):
                    list_inserted += 1
                else:
                    list_updated += 1
            db.commit()
    ...
    if missing:
        ...
        for i, (pn,) in enumerate(missing):
            ...
            if detail:
                _upsert_posting(db, detail, store_id)
                get_inserted += 1
            else:
                get_failed += 1
            if (i + 1) % 50 == 0:
    ...
    logger.info(
        f"[store={store_id}] 订单履约同步完成: list={list_inserted}+{list_updated}, "
