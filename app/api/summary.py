...
def list_summary(
    ...
    for s, name, primary_image, offer_id in rows:
        result.append(SummaryItem(
            ...
            cancelled_units=s.cancelled_units,
            revenue=s.revenue,
            ...
            promotion_costs=s.promotion_costs,
...
def summary_stats(
    ...
    q = db.query(
        ...
        func.sum(SkuDailySummary.delivered_units).label("total_delivered_units"),
        func.sum(SkuDailySummary.cancelled_units).label("total_cancelled_units"),
        ...
        func.sum(SkuDailySummary.returns_units).label("total_returns_units"),
        func.sum(SkuDailySummary.storage_fees).label("total_storage"),
        func.sum(SkuDailySummary.advertising).label("total_advertising"),
        func.sum(SkuDailySummary.promotion_costs).label("total_promotion"),
    ...
    stats = SummaryStats(
        ...
        total_delivered_units=int(row.total_delivered_units or 0),
        total_cancelled_units=int(row.total_cancelled_units or 0),
        ...
        total_returns_units=int(row.total_returns_units or 0),
        total_storage=Decimal(str(row.total_storage or 0)),
        total_advertising=Decimal(str(row.total_advertising or 0)),
        total_promotion=Decimal(str(row.total_promotion or 0)),
