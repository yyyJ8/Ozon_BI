...
fillable = c.execute(text("""
    ...
    WHERE s.record_date = '2026-08-21' AND s.green_price IS NULL
      AND m.green_price_rub IS NOT NULL
""")).scalar()
total_20 = c.execute(text("SELECT COUNT(*) FROM ozon.sku_daily_snapshot WHERE record_date='2026-08-21'")).scalar()
already = c.execute(text("SELECT COUNT(*) FROM ozon.sku_daily_snapshot WHERE record_date='2026-08-21' AND green_price IS NOT NULL")).scalar()
...
r = c.execute(text("""
    ...
    FROM ozon.sku_daily_snapshot WHERE record_date='2026-08-21' GROUP BY record_date
""")).fetchone()
...
if not DRY_RUN:
    ...
    for row in c.execute(text("""
        ...
        WHERE s.record_date='2026-08-21' AND s.green_price IS NOT NULL
